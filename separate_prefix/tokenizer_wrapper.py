...
class IMDBLMTokenizerWrapper(TokenizerWrapper):
    r"""
    ...
    def tokenize_one_example(self, wrapped_example, teacher_forcing):
        ...
        # pad to max length
        input_ids_len = min(self.max_seq_length, len(encoder_inputs['input_ids']))
        encoder_inputs = self.padding_1(
    ...
    def padding_1(self, input_dict: Dict,
                  max_len: int, pad_id_for_inputs: int = 0, pad_id_for_others: int = 0) -> None:
        for key, value in input_dict.items():
            if len(value) > max_len - 1:
                if 'input' in key:
                    input_dict[key] = value[:max_len - 1] + [50256]
                else:
                    input_dict[key] = value[:max_len]
            if 'input' in key:
