...
class MultiPrefixTuningTemplate(Template):
    ...
    def __init__(self,
                 ...
                 num_types: Optional[int] = 150,
                 shared_num_token: Optional[int] = 0,
                 num_token_per_type: Optional[int] = 2,
        ...
        self.num_token_per_type = num_token_per_type
        self.shared_num_token = shared_num_token
        self.num_token = num_types * num_token_per_type + shared_num_token
    ...
    def get_past_key_values_step1(self, batch_size=1, labels=None):
        pvs = []
        ...
        if (not self.config.is_encoder_decoder) or self.using_decoder_past_key_values:
            decoder_input_tokens = (labels.unsqueeze(1) * self.num_token_per_type).expand(-1,
                                                                                          self.num_token_per_type) + torch.arange(
                self.num_token_per_type).to(self.input_tokens.device)
            if self.shared_num_token:
                shared_decoder_input_tokens = self.shared_input_tokens.unsqueeze(0).expand(batch_size,
                                                                                           -1)
                decoder_input_tokens = torch.cat([decoder_input_tokens, shared_decoder_input_tokens], dim=1)
    ...
    def get_past_key_values_step2(self, batch_size=1, labels=None):
        pvs = []
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            input_tokens = self.input_tokens.unsqueeze(0).expand(batch_size, -1)
            temp_control = self.wte(input_tokens)
    ...
    def get_past_key_values_step3(self, batch_size=1, labels=None):
        pvs = []
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            input_tokens = self.input_tokens.unsqueeze(0).expand(batch_size, -1)
            temp_control = self.wte(input_tokens)
        ...
        if (not self.config.is_encoder_decoder) or self.using_decoder_past_key_values:
            ...
            decoder_past_key_values = self.decoder_control_trans(decoder_temp_control)
            _, decoder_seqlen, _ = decoder_past_key_values.shape
            decoder_past_key_values = decoder_past_key_values.view(1, decoder_seqlen, self.match_n_decoder_layer * 2,
                                                                   self.match_n_head,
                                                                   self.match_n_embd)
            past_decoder_past_key_values = decoder_past_key_values.detach().squeeze()

            decoder_input_tokens = (labels.unsqueeze(1) * self.num_token_per_type).expand(-1,
                                                                                          self.num_token_per_type) + torch.arange(
                self.num_token_per_type).to(self.input_tokens.device)
            if self.shared_num_token > 0:
                shared_decoder_input_tokens = self.shared_input_tokens.unsqueeze(0).expand(batch_size,
                                                                                           -1)
                decoder_input_tokens = torch.cat([decoder_input_tokens, shared_decoder_input_tokens], dim=1)
            decoder_temp_control = self.decoder_wte(decoder_input_tokens)
            ...
            list_decoder_input_tokens = decoder_input_tokens.tolist()
            reversed_index = [list(set(self.input_tokens.tolist()) - set(list_decoder_input_tokens[index])) for index in
                              range(batch_size)]
            reversed_index = torch.tensor(reversed_index, dtype=torch.long, device=self.input_tokens.device)
            reversed_past_decoder_past_key_values = past_decoder_past_key_values[reversed_index.view(-1)].view(
                batch_size, -1, 24, 12, 64)
            decoder_past_key_values = torch.cat((decoder_past_key_values, reversed_past_decoder_past_key_values), dim=1)
    ...
    def get_past_key_values_step4(self, batch_size=1, labels=None):
        pvs = []
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            input_tokens = self.input_tokens.unsqueeze(0).expand(batch_size, -1)
            temp_control = self.wte(input_tokens)
        ...
        if (not self.config.is_encoder_decoder) or self.using_decoder_past_key_values:
            shared_decoder_input_tokens = self.shared_input_tokens.unsqueeze(0).expand(batch_size, -1)
            decoder_temp_control = self.decoder_wte(shared_decoder_input_tokens)
            decoder_past_key_values = self.decoder_control_trans(decoder_temp_control)
            _, decoder_seqlen, _ = decoder_past_key_values.shape
            decoder_past_key_values = decoder_past_key_values.view(batch_size, decoder_seqlen,
                                                                   self.match_n_decoder_layer * 2, self.match_n_head,
    ...
    def set_work_stage(self, stage=1):
        self.work_stage = stage
        if stage == 1:
            self.get_past_key_values = self.get_past_key_values_step1
        elif stage == 2:
            self.get_past_key_values = self.get_past_key_values_step2
        elif stage == 3:
            self.get_past_key_values = self.get_past_key_values_step3
        elif stage == 4:
            self.get_past_key_values = self.get_past_key_values_step4
        else:
    ...
    def generate_parameters(self) -> None:
        ...
        if self.shared_num_token:
            self.shared_input_tokens = nn.Parameter(
                torch.arange(self.num_types * self.num_token_per_type, self.num_token).long(), requires_grad=False)
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        labels = batch['label'].to(self.input_tokens.device)
        ...
        if self.config.is_encoder_decoder:
            ...
            if self.work_stage in [1]:
                self.past_key_values = self.get_past_key_values(batch_size=batch_size, labels=labels)
                past_key_values = self.past_key_values[1]
                if 'attention_mask' in batch:
                    am = batch['attention_mask']
                    batch['attention_mask'] = torch.cat([torch.ones(
                        (batch_size, self.num_token_per_type + self.shared_num_token), dtype=am.dtype,
                        device=am.device), am], dim=-1)
            elif self.work_stage in [2, 3]:
                self.past_key_values = self.get_past_key_values(batch_size=batch_size, labels=labels)
                past_key_values = self.expand_to_batchsize(self.past_key_values[1], batch_size)
                if 'attention_mask' in batch:
                    am = batch['attention_mask']
                    batch['attention_mask'] = torch.cat(
                        [torch.ones((batch_size, self.num_token), dtype=am.dtype, device=am.device), am], dim=-1)
                    # position_ids = torch.arange(self.num_token_per_type, am.size(1) + self.num_token_per_type, dtype=am.dtype, device=am.device).unsqueeze(0).expand(batch_size, -1)
                    # setattr(batch, 'position_ids', position_ids)
            elif self.work_stage in [4]:
                self.past_key_values = self.get_past_key_values(batch_size=1)
                past_key_values = self.expand_to_batchsize(self.past_key_values[1], batch_size)
                if 'attention_mask' in batch:
                    am = batch['attention_mask']
                    batch['attention_mask'] = torch.cat(
                        [torch.ones((batch_size, self.shared_num_token), dtype=am.dtype, device=am.device), am], dim=-1)
                ...
                for i, layer_module in enumerate(model.encoder.block):
                    ...
                    def modified_encoder_forward(*args, **kwargs):
                        ...
                        if kwargs['past_key_value'] is None:
                            kwargs['past_key_value'] = self.expand_to_batchsize_for_layer(self.past_key_values[0],
                                                                                          batch_size, layer_id).to(
                                device)
            ...
            if self.using_decoder_past_key_values:
                ...
                for i, layer_module in enumerate(model.decoder.block):
                    ...
                    def modified_decoder_self_attn_forward(*args, **kwargs):
                        ...
                        device = args[0].device
                        if kwargs['past_key_value'] is None:
                            kwargs['past_key_value'] = self.expand_to_batchsize_for_layer(self.past_key_values[1],
                                                                                          batch_size, layer_id).to(
                                device)
